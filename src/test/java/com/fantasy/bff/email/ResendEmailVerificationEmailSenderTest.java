...
class ResendEmailVerificationEmailSenderTest {

    private static final Instant EXPIRES = Instant.now().plusSeconds(86400);
    private static final String LINK = "https://app/verify-email?token=live-verify-token";

    private static ResendEmailVerificationEmailSender sender(String apiKey, String baseUrl, boolean logLinks) {
        return new ResendEmailVerificationEmailSender(new EmailProperties(
    ...
    void doesNotSendOrLogTheLink_whenApiKeyUnsetOutsideALocalRun(CapturedOutput output) {
        ...
        try {
            ...
            assertThat(output).doesNotContain("live-verify-token").contains("was not sent");
        } finally {
            server.stop();
        }
    }
    ...
    void postsToResendWithBearerAuth_whenApiKeySet(CapturedOutput output) {
        ...
        try {
            ...
            assertThat(output).doesNotContain("live-verify-token");
        } finally {
            server.stop();
        }
    }
